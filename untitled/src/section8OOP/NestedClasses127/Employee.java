package section8OOP.NestedClasses127;
...
/**
 * Section 8 More OOP - 127 Nested Classes
 *
 * On a besoin d'une fausse class pour remplacer les cases null dans le switch (createEmployee()) qui si ils ce produisent plante le programme
 * car on ne peut pas appeler methode (getSalaty() ou toString()) à partir d'un objet null
 * Pour régler ce probléme on décide de créer une fausse class DummyEmployee
 * Mais comme il y a presque rien dessus il crée cette class à l'interieur de la super-class Employee on parle alors de Nested Class
 */
abstract  class Employee {// abstract class => on ne veut permettre a personnes d'instancier une abstract class | flag pour dire qu'il y aura une abstract methode qui devra etre implementé par les class filles
    ...
    //____________2____________//** IDE nous demande de crée un constructeur vide, DummyEmployee va hériter de ce constructeur vide
    ...
    //____________5____________//** Met ce constructor protected => ça forcera les dev a utiliser seulement createEmployee() pour crée de instance Employee plutot que ce constructor
    ...
    public static final Employee createEmployee(String employeeText) {
        ...
        if (peopleMat.find()) {
            return switch (peopleMat.group("role")) { // Il a changé le type de return du switch ce n'est plus le salaire (totalSalaries+=) int qui est retourné mais type Employee (interface) stocké dans une variable de ce type => vire les { } | pour chaque ligne on utilise un switch pour instancier quel class on a besoin en fct du role (poste)
                ...
                //____________3____________//** Remplace les null par instanciation de DummyEmployee pour éviter que le programme plante si la methode createEmployee return null | On ne peut pas appler de methode depuis un null
    ...
    //____________1____________//** Crée une fausse class pour avoir un objet pour remplacer les null case dans le switch createEmployee() car si cette methode return null le programme peut plater on ne peut lancer des methode depuis un null
