...
/**
 * Section 8 More OOP - 132 Default Methods
 *
 * Autre methode pour contourner le manque de prise en charge traditionnelle de l'héritage multiple en Java
 * On ne peut pas le faire avec l'héritage car une class ne peut extends qu'1 seule autre class
 * Mais déconseille de le faire en production (utilisé par ceux qui crée des API = interface de programmation d'application, Frameworks)
 * Si vraiment besoin d'héritage multiple recommande plutot Composition
 *
 * On rajoute des méthodes implémentées sur les interfaces avec le mot clé default
 * La class qui implémente cette interface n'a pas l'obligation d'implémenter ces méthodes
 * Mais c'est comme si la class les pocédaient réellement car son instance à les méthodes implémentées de l'interface à sa disposition
 *
 * Différence entre class abstract et defaut methods sur une interface :
 * Une class abstract => ne peut pas etre instancié seul mais peut avoir des fields (peu maintenir états)
 * defaut methods interface => les fields sont rendu final (ne peut pas maintenir un etat) et donc peu utile
 *
 *
 */

//__________2__________ La class Programmer implemente cette interface Chef (qui a des méthodes implémentées)
