...
/**
 * Section 8 More OOP - 133 Comparing Classes with 'instanceof'
 * Vérifier si un Objet fait parti du meme groupe hiérarchique (ex interface == subclass)
 *
 * Ici on veut print les details qui sont différents si l'employee est Programmer, Manager, ...
 * RQ on utilise if et non pas un switch car un switch ne fonctionnent que pour les primitifs, les wrapper de primitifs (Integer,..) et les Enums.
 *    Les switch ne peuvent même pas être utilisés pour d'autres types de données. Dans ce cas, une interface ou une instance de classe.
 *    Mais depuis Java 17 le switch fctionne pour tous
 *
 * Methode 1 :
 * if (employee.Class().equals(Programmer.class)  // getClass() => tous les Object hérite de cette méthode | equals() => permet de tester si c'est égale à une autre class (ne pas oublié .class à la fin de la class à comparé)| dangé si employee est null plante le programme si appel une methode depuis un null => technique Optinal
 *
 * Methode 2.1 (old school pour ceux qui utilise Eclipse)
 * if (employee instanceof Programmer) {
 *      Programmer prog = (Programmer) employee // il prend cet employee variable le cast en Programmer et stock ça dans une nell local variable prog de type Programmer
*      System.out.printf("Nombre de ligne de code = %d, Nbre d'années d'expérience = %d, QI = %d %n",prog.getLinesOfCode(), prog.getYearsOfExp(), prog.getIq());  //  on utilise cette nell variable locale prog pour lancer les methode de Programmer
 *
 *
 * Methode 2.2 (pour ceux qui utilise ItelliJ permet un racourci)
 * if (employee instanceof Programmer) {
 *      System.out.printf("Nombre de ligne de code = %d, Nbre d'années d'expérience = %d, QI = %d %n",((Programmer) employee).getLinesOfCode(), ((Programmer) employee).getYearsOfExp(), ((Programmer) employee).getIq()); // C'est la meme chose en un peut plus cours cast employee qui est de type IEmployee en Programmer avant d'appeler ces méthodes
 *
 *
 * Methode 2.3 (depuis Java 14 IntelliJ un racorci encore amélioré qu'on appel Pattern matching)
 * if (employee instanceof Programmer prog) {  // En rajoutant juste un nom de variable prog IntelliJ fait auto le cast pour nous
