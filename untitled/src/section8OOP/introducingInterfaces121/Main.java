...
/**
 * Section 8 More OOP - 121 Introducing Interfaces
 *
 * Suite => Factorisation du code grace au concept d'Interface pour calculer la somme des salaires que doit payer l'entreprise avec des methode de calcul différente our chaque role (job)
 * Le code dupliqué n'est souvent pas une bonne chose
 *
 * Factorise les switch case en utilisant une interface
 * + instancie les class dans le switch avec ce type Employee (interface)
 * Toutes les instances sont du meme type Employee (interface)
 * => le switch ne retourne une instance de type Employee (interface) stock dans une variable de type Employee (interface)
 * Cette variable de type Employee (interface) dispose de la methode getSalary() qui fait le calcul du salaire
 *
 * - Vous pouvez donner l'impression que ces classes (instances) sont toutes du même type en plaçant une interface dessus, `implement` EX les class ont tous cette CAPACITE à calculer un salaire. =>
 *
 * - On pourrait penser, cette interface comme un masque car une fois que l'instance d'une class est de type interface son instance ne pourra utiliser que les methode de l'interface et non plus de toute la class
 *
