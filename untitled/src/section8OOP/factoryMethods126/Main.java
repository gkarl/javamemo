 ...
 *
 * Suite => Factorisation du code encore plus pour calculer la somme des salaires que doit payer l'entreprise avec des methode de calcul différente our chaque role (job)
 *
 * - Il pense que le switch se rapporte plus à la class Employee qu'a la class Main car instancie (Programmer, Manager ..) sub-class de la super class Employee
 * Cela ne concerne pas la Class Main qui dit juste "donne moi le salaire"
 * On veut le plus possible faire que chaque class face le minimum de chose (1 seul chose si possible) => place le code au bon endroit
 *
 * - Comment aussi peopleRegex peoplePattern dans le Main car déja présent dans la super class Employee
 */
class Main {
    ...
    public static void main(String[] args) {
        ...
        //___________9______________ //** ETAPE 2 Il trouve que ces 2 ligne de code peopleRegex et peoplePattern sont du code redondant car déja présent | et pour que ça passe remplace peoplePattern => Employee.peoplePattern 3em ligne
        ...
        while (peopleMat.find()) { // ça va passer (boucle) sur chaque ligne du texte people
            //___________8______________ //** remplace le switch par un appel à la methode createEmployee() qui le contiend
            employee = Employee.createEmployee(peopleMat.group());
                    //___________0______________ //** ETAPE 1 Il commente le switch qu'il a copié dans la methode createEmployee() qu'il a mis sur la class Employee place plus adapté
