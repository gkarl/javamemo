package section8OOP.otherTypesOfNestedClasses128;
...
/**
 * Section 8 More OOP - 128 Other Types of Nested Classes
 *
 * Il existe plusieurs type de Nested Class
 *
 * - 1/ Static Nested Class => Qu'on a vu à la leçon précédente | Elle ne permet pas l'accés aux fields (et methode non static) de la class englobante
 *
 * - 2/ Inner Class => Permet pas l'accés aux fields (et methode non static) de la class englobante (Utilise si on en a besoin)
 *      EX :
 *      private final class MyInnerClass {  // RQ identique à DummyEmployee (Static Nested Class) juste retire le mot clé static
 *          public int getStuff() {
 ...
 *      };
 *
 * - 4/ Local Nested Class => C'est une class à l'interieur d'une méthode dans la super-class RQ un peu comme une variable local
 *      EX :
 *      public static final Employee createEmployee(String employeeText) {
 *          ...
 *          class MyLocalClass extends Employee {
 *              public int getSalary() {  // Comme MyLocalClass hérite de Employee elle doit implémenter la methode abstraite getSalary()
 *                  return 5;
 *              }
 *          }
 *          ...
 *          } else {
 *          return new MyLocalClass(); // Cette Class local à l'intérieur de cette methode peut etre instancier localement dans cette meme méthode
 *      }
 *
 *    RQ les 3 derniers type de Nested Class sont tres peu utilisé et ne servent pas notre programme c'est pour cela qu'on ne les implémentera pas
 */
