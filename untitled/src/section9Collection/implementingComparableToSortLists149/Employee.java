...
/**
 *  On a utilisé Lambda expression à 2 endroits , c'est juste une fct. Java prendra cette fonction et essaiera de la mapper à une seule méthode de l'interface (IEmployee) qui correspondant à cette signature de methode
 *
 * EX signature de la methode recherché dans 1 interface qui ne prend aucun arg et return un int
 *
 * Maintenant, afin de savoir à quelle méthode d'1 interface cette Lambda peut mapper, nous devons aider Java à déduire l'interface (type de données IEmployee return de la methode createEmployee() qui contiend la Lambda qui est aussi un return de createEmployee()  donc Lambda est de type IEmployee)
 * => Java déduit que la Lambda doit mapper IEmployee
 *
 * - Si il y a 1 seul méthode dans interface IEmployéé
 * - et si la signature de la methode dans interface IEmployee match la signature de la Lambda => mappe la Lambda avec la methode getSalary() qui est défini sur l'interface IEmployee
 *
 * PB si IEmployee à plus d'1 methode définit en elle (elle n'est plus fonctional) Java ne sait plus comment faire le mappage
 */
